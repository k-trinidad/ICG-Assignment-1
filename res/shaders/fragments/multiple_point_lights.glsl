...
vec3 CalcSpecContribution(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess) {
	// Get the direction to the light in world space
	vec3 toLight = light.Position.xyz - worldPos;
	// Get distance between fragment and light
	float dist = length(toLight);
 ...
 * @param camPos The camera's position in world space
*/
vec3 CalcSpecOnly(vec3 worldPos, vec3 normal, vec3 camPos, float shininess) {
    // Will accumulate the contributions of all lights on this fragment
	vec3 lightAccumulation = AmbientColAndNumLights.rgb;
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		// Additive lighting model
		lightAccumulation += CalcSpecContribution(worldPos, normal, viewDir, Lights[ix], shininess);
	}

	return lightAccumulation;
}

vec3 CalcToonShading(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess) {
	// Get the direction to the light in world space
	vec3 toLight = light.Position.xyz - worldPos;
	// Get distance between fragment and light
	float dist = length(toLight);
	...
	else if (diffuseFactor >= 0.4 && diffuseFactor < 0.7) {
		diffuseFactor = 0.7;
	}
	else {
		diffuseFactor = 1;
...
vec3 CalcToonShadingOnly(vec3 worldPos, vec3 normal, vec3 camPos, float shininess) {
    // Will accumulate the contributions of all lights on this fragment
	vec3 lightAccumulation = AmbientColAndNumLights.rgb;
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		// Additive lighting model
		lightAccumulation += CalcToonShading(worldPos, normal, viewDir, Lights[ix], shininess);
	}

	return lightAccumulation;
}

vec3 CalcRampDiffuse(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess, float diffuseRamp) {
	// Get the direction to the light in world space
	vec3 toLight = light.Position.xyz - worldPos;
	// Get distance between fragment and light
	float dist = length(toLight);
	...
	// Calculate diffuse color
	vec3  diffuseOut = diffuseRamp * light.ColorAttenuation.rgb;
...
vec3 CalcRampDiffuseOnly(vec3 worldPos, vec3 normal, vec3 camPos, float shininess, float diffuseRamp) {
    // Will accumulate the contributions of all lights on this fragment
	vec3 lightAccumulation = AmbientColAndNumLights.rgb;
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		// Additive lighting model
		lightAccumulation += CalcRampDiffuse(worldPos, normal, viewDir, Lights[ix], shininess, diffuseRamp);
	}

	return lightAccumulation;
}

vec3 CalcRampSpec(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess, float specRamp) {
	// Get the direction to the light in world space
	vec3 toLight = light.Position.xyz - worldPos;
	// Get distance between fragment and light
	float dist = length(toLight);
	...
	float specPower  = pow(max(dot(normal, halfDir), 0.0), pow(256, shininess));
	// Calculate specular color
	vec3 specularOut = specRamp * light.ColorAttenuation.rgb;
...
vec3 CalcRampSpecOnly(vec3 worldPos, vec3 normal, vec3 camPos, float shininess, float specRamp) {
    // Will accumulate the contributions of all lights on this fragment
	vec3 lightAccumulation = AmbientColAndNumLights.rgb;
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		// Additive lighting model
		lightAccumulation += CalcRampSpec(worldPos, normal, viewDir, Lights[ix], shininess, specRamp);
	}
